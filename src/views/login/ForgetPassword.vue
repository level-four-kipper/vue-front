    ...
    <div class="login">
      ...
      <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-position="top">
        <el-form-item label="新密码" prop="newPassword">
          <el-input v-model="ruleForm.newPassword" type="password" autocomplete="new-password" :readonly="readonlyFlag" @focus="readonlyFlag = false" show-password clearable placeholder="请输入新密码"></el-input>
        </el-form-item>
        ...
        <el-button class="submit-btn" :loading="submitLoading" @click="submitForm(ruleFormRef)" type="primary">{{'重置密码'}}</el-button>
...
const rules = reactive({
  ...
  confirmPassword: [{ required: true, validator: validateRepressPassword, trigger: 'blur' }],
...
const handleSendCode = () => {
  ...
  count.value = 59
  handleCountDown()
  fetchCheckCode(route.query.email).then(res => {
    if (res.code !== 0) {
      clearInterval(timer)
